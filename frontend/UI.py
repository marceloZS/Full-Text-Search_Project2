...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Configurar el fondo
        self.fondo = QtGui.QPixmap('nuevo_fondo.png')
        ...
        logo = logo.scaled(logo.width() // 2, logo.height() // 2, QtCore.Qt.KeepAspectRatio)
        ...
        self.entrada.setStyleSheet("QLineEdit { color: #ff00ff; font-size: 16px; font-weight: bold; text-shadow: 2px 2px 4px #00ffff; }")
        self.boton_buscar = QPushButton("Buscar", self)
        self.boton_buscar.setStyleSheet("QPushButton { color: #00ff00; font-size: 16px; font-weight: bold; text-shadow: 2px 2px 4px #ffff00; }")
        self.boton_buscar.clicked.connect(self.buscar)
        self.entrada_numero = QLineEdit(self)
        self.entrada_numero.setStyleSheet("QLineEdit { color: #ff00ff; font-size: 16px; font-weight: bold; text-shadow: 2px 2px 4px #00ffff; }")


        self.cuadro1 = QLabel("Resultado 1: ", self)
        self.cuadro1.setStyleSheet("QLabel { color: #ffff00; font-size: 16px; font-weight: bold; text-shadow: 2px 2px 4px #00ffff; background-color: #000000; }")

        self.cuadro2 = QLabel("Resultado 2: ", self)
        self.cuadro2.setStyleSheet("QLabel { color: #ffff00; font-size: 16px; font-weight: bold; text-shadow: 2px 2px 4px #ffff00; background-color: #000000; }")
        ...
        self.layout.addWidget(self.boton_buscar, 0, 1)
        self.layout.addWidget(self.entrada_numero, 0, 2)
    ...
    def buscar(self):
        ...
        self.resultados_text1.clear()  # Clear previous results

        # Get the inserted number
        k = int(self.entrada_numero.text())

        for i, fila in enumerate(resultados):
            if i < k:  # Show only the first k results
